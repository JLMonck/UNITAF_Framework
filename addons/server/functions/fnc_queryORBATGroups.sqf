...
private _query = "extDB3" callExtension format["0:FETCHDATA:SELECT ol.unit, CONCAT(oc.callsign,' ',u.append) as callsign, 'WEST' as side FROM operation_layout ol LEFT JOIN operation_unit_callsigns olc ON olc.unit = ol.unit LEFT JOIN operation_callsigns oc ON oc.id = olc.callsign LEFT JOIN units u ON u.id = ol.unit LEFT JOIN users us ON us.id = ol.user WHERE ol.operation IN ('%1') AND olc.operation = ol.operation GROUP BY ol.unit", _operationID];
