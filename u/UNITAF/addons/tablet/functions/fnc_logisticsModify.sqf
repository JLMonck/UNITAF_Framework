...
private _cargo = [_display, _item] call FUNC(logisticsGetCargo);
...
if (_addItem && {_maximumLoad - _currentLoad >= _mass}) then {
    ...
    if (_index == -1) then {
        ...
        _count = (_counts select _index) + _modify;
...
if (!_addItem && {_index != -1}) then {
    private _count = (_counts select _index) - _modify;
