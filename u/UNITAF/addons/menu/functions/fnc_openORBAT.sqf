...
// show ORBAT control
GET_ORBAT(_display) ctrlShow true;
GET_BACK(_display) ctrlShow true;

// Clear table when opening (again)
ctClear GET_IN_ORBAT(_display,IDC_ORBAT_TABLE);
...
_prevCallsign = "";
{
	if !(_prevCallsign isEqualTo (_x select 4)) then {
		_headerCtrls = ctAddHeader GET_IN_ORBAT(_display,IDC_ORBAT_TABLE);
	...
	_rowCtrls = ctAddRow GET_IN_ORBAT(_display,IDC_ORBAT_TABLE);
 ...
 GET_IN_ORBAT(_display,IDC_ORBAT_TABLE) ctrlSetPosition [0, 0, 1, (((((safezoneW / safezoneH) min 1.2) / 1.2) / 25) * _totalHeight)];